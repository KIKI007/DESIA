...
void draw_puzzle(std::shared_ptr<VoxelizedInterface> interface, const struct Parameter &para)
{
    if(interface)
    {
...
void read_puzzle_file(const struct Parameter &para)
{
    ...
    if(file_name != "")
    {
        ...
        array_3i array(boost::extents[Nx][Ny][Nz]);
...
void automatic_search(struct Parameter &para)
{
...
void go_back(struct Parameter &para)
{
    if(voxel_tree != nullptr && voxel_tree->present_nodes_ != nullptr)
    {

...
namespace voxel{
    ...
    {
        std::string path = CRL_DATA_FOLDER "/mesh/sphere.obj";
        if(igl::readOBJ(path, sphere.V, sphere.F))
        {
            double scale = sphere_radius / 3;
            sphere.V *= scale;
            return true;
        }
        else
    ...
    {
        std::string path = CRL_DATA_FOLDER "/mesh/xcylinder.obj";
        if(igl::readOBJ(path, cylinderX.V, cylinderX.F))
        {
            double scale = cylinder_length / 10.0;
            for(int id = 0; id < cylinderX.V.rows(); id++)
            {
                cylinderX.V(id, 0) *= scale;
                cylinderX.V(id, 1) *= 0.2;
                cylinderX.V(id, 2) *= 0.2;
            }
            return true;
        }
        else
    ...
    {
        std::string path =  CRL_DATA_FOLDER "/mesh/zcylinder.obj";
        if(igl::readOBJ(path, cylinderY.V, cylinderY.F))
        {
            double scale = cylinder_length / 12;
            for(int id = 0; id < cylinderY.V.rows(); id++)
            {
                cylinderY.V(id, 0) *= 0.2;
                cylinderY.V(id, 1) *= scale;
                cylinderY.V(id, 2) *= 0.2;
            }
            return true;
        }
        else
    ...
    {
        std::string path = CRL_DATA_FOLDER "/mesh/ycylinder.obj";
        if(igl::readOBJ(path, cylinderZ.V, cylinderZ.F))
        {
            double scale = cylinder_length / 12;
            for(int id = 0; id < cylinderZ.V.rows(); id++)
            {
                cylinderZ.V(id, 0) *= 0.2;
                cylinderZ.V(id, 1) *= 0.2;
